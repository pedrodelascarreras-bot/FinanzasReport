...
(function(){
  ...
  function renderBalanceTopMetrics(data){
    ...
    if(incomeEl) incomeEl.textContent = balanceFmtMoney(data.income.total);
    if(expensesEl) expensesEl.textContent = balanceFmtMoney(data.totalExpenses);
    if(netEl){
      netEl.textContent = balanceFmtMoney(data.freeCash);
      netEl.style.color = data.freeCash < 0 ? 'var(--danger)' : (data.freeCash === 0 ? 'var(--text)' : 'var(--accent2)');
    }
  }
